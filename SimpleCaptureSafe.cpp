...
#define DEFAULT_IMAGE_HEIGHT    964
/*
 * default amount of frames to capture...can also specify on command line
 */
#define DEFAULT_NUM_FRAMES     100 //0xffffffff
...
extern "C" int sfrProc(double **,double **, int*, double *, 
	...
	else 
	{
		for (i = 0; i < data->row; ++i) {
			for (j = 0; j < data->col; ++j) {
				...
				if (data->matrix[i][j]>65000) { printf("Saturated pixel [%d][%d]=%d\n",j,i,data->matrix[i][j]); }
			}
		}
	}
	...
	if (data->max_gray > 255)
	{
		for (i = y; i < y+ height; ++i)
		{
			for (j = x; j < x + width; ++j)
			{
				...
				if (pixdata > 65000) { printf("Sat pixel[%d][%d]=%d\n",j,i,pixdata); }
...
void ImageWrite(cv::string filename, cv::Mat ImageData)
{
	try {
...
cv::vector<cv::Point2f> lineToPointPair(cv::Vec2f line)
{
...
int LocateMultipleFeaturesDetection2(cv::Mat img, cv::Mat img_display, int width, int height, const flags_t modeflags)
{
	...
	if (((modeflags.WriteImageLevel > 3)&& (lKey == NO_KEY_PRESSED)) || (((lKey == 0x73)|| (lKey == 0x53))&& (modeflags.WriteImageLevel == 3)))
...
bool InitializeROIs(string filePath)
{
	...
	ConfigurationXML.open(filePath + "\\Lens.xml", cv::FileStorage::READ);
	...
	cv::FileNode Boxes_Node = ConfigurationXML["FOCUS_SFR_11Boxes"];

	for (int boxNumber = 0; boxNumber < MAX_NUMBER_BOXES; boxNumber++)
	{
		cv::string BoxValue("Box");
		...
		for(int i = 0;i<ROI_PER_SLANTBOX;i++)
		{
			...
			{
			case 0 : //North
				yoffset = -(Boxes[boxNumber].SB_Size/2)-(Boxes[boxNumber].SFRBox[i].roi.height/2);
				xoffset = -(Boxes[boxNumber].SFRBox[i].roi.width/2);	
				break;
			case 2 : //south
				yoffset = (Boxes[boxNumber].SB_Size/2)-(Boxes[boxNumber].SFRBox[i].roi.height/2);
				xoffset = -(Boxes[boxNumber].SFRBox[i].roi.width/2);
				break;
			case 1 : // EAST
				yoffset = -(Boxes[boxNumber].SFRBox[i].roi.height/2);
				xoffset = (Boxes[boxNumber].SB_Size/2)-(Boxes[boxNumber].SFRBox[i].roi.width/2);
				break;
			case 3 : //West
				yoffset = -(Boxes[boxNumber].SFRBox[i].roi.height/2);
				xoffset = -(Boxes[boxNumber].SB_Size/2)-(Boxes[boxNumber].SFRBox[i].roi.width/2);
...
double CalcSFR_Hi(wchar_t* ptr_img, Roi_for_SFR & SFRBox, int width, int height, int grey_Level, double MM_PER_PIX, const flags_t modeflags)
{
	// added for SFR
	int i;
	double *farea;
	...
	int g_version = 0;	// flag to use different alg variation
						// leave at zero.
	...
	if (!(SFRBox.roi.y % 2))
	{
		...
		else if (modeflags.lEmbeddedData == 2)
		{
			if (SFRBox.isHorz)
				odd_y_offset = 1;
			else
			...
			cout<< "Undefined embedded data definition" << endl;
	}
	else
	{
		if (modeflags.lEmbeddedData == 1)
	...
	if (modeflags.Demosaic == 0)
	{
		if(!SFRBox.isHorz)
		{
			...
			cv::Mat roi_file2(size_y/2, size_x,CV_16U,cv::Scalar::all(0));
			// Vert SFR Calc - read rows col by col and convert to ratiometric (0 - 1) array.
		...
		else
		{
			...
			cv::Mat roi_file2(size_y,(size_x/2),CV_16U,cv::Scalar::all(0));
	...
	err = sfrProc(&Freq, &disp, &bin_len, farea, (unsigned short)size_x, &size_y, &slope, &numcycles,&center,&off, &R2, g_version, 0, 0);
...
void Score_Image(int argc, char* argv[], const flags_t modeflags)
{
	...
	if (((szChar[lLength-1] == 'm') || (szChar[lLength-1] == 'M')) && ((szChar[lLength-2] == 'g') || (szChar[lLength-2] == 'G')) && ((szChar[lLength-3] == 'p')|| (szChar[lLength-3] == 'P')))
	{
		...
		{
			for (int j = 0; j < dataArray_Hi_16BitsPerPixel->col/2; ++j)
			{
				int temp = dataArray_Hi_16BitsPerPixel->matrix[i][j];
				dataArray_Hi_16BitsPerPixel->matrix[i][j] = dataArray_Hi_16BitsPerPixel->matrix[i][dataArray_Hi_16BitsPerPixel->col -1 -j];
				dataArray_Hi_16BitsPerPixel->matrix[i][dataArray_Hi_16BitsPerPixel->col -1 -j] = temp;
			}
		}
		writePGM_16("Test.pgm", dataArray_Hi_16BitsPerPixel,0,0,dataArray_Hi_16BitsPerPixel->row,dataArray_Hi_16BitsPerPixel->col,dataArray_Hi_16BitsPerPixel->max_gray,false);
	}
	else if (((szChar[lLength-1] == 'g') || (szChar[lLength-1] == 'G')) && ((szChar[lLength-2] == 'n') || (szChar[lLength-2] == 'N')) && ((szChar[lLength-3] == 'p')|| (szChar[lLength-3] == 'P')))
	{
		...
		dataArray_Hi_16BitsPerPixel->max_gray = (int)pow(2.0,lBits_Per_Pixel) ;
	...
	for (int i = 0; i < dataArray_Hi_16BitsPerPixel->row; ++i)
	{
		...
		{
			// Division by 16 because the data are store in the 12 MSB not the 12 LSB
			dataArray_Hi_12BitsPerPixel->matrix[i][j] = (dataArray_Hi_16BitsPerPixel->matrix[i][j])>>4; 
		}		
	}
	...
	{
		writePGM_16(szChar, dataArray_Hi_12BitsPerPixel,0,0,dataArray_Hi_12BitsPerPixel->row,dataArray_Hi_12BitsPerPixel->col,dataArray_Hi_12BitsPerPixel->max_gray);
	...
	cv::Mat lImageDataDisplay_12BitsPerPixel = cv::imread(szChar_DD, CV_LOAD_IMAGE_ANYDEPTH);
	if (lImageDataDisplay_12BitsPerPixel.rows == 0 )
	{
		cout << "Error reading the 12 bits Per Pixel Display Image" << endl;
	...
	case 2:
		{
			...
			//lImageDataDisplay_12BitsPerPixel.convertTo(lImageDataDisplay_8BitsPerPixel,CV_8U);
			for (int i = 0; i < lImageDataDisplay_12BitsPerPixel.rows; ++i)
			{
				for (int j = 0; j < lImageDataDisplay_12BitsPerPixel.cols; ++j)
				{
	...
	case 3:
		{
			...
			if(!lSaturated)
			{
				LocateMultipleFeaturesDetection2(lImageDataDisplay_8BitsPerPixel, lImageDataDisplay_12BitsPerPixel, lImageDataDisplay_8BitsPerPixel.cols, lImageDataDisplay_8BitsPerPixel.rows, modeflags);
			}
		}
		break;
	default:
	...
	if (modeflags.lTargetDetection != 3)
	{
		...
		InitializeROIs(szCharWithoutExePath);
	}
	else
	{
		...
		for (int boxNumber = 0; boxNumber < MAX_NUMBER_BOXES; boxNumber++)
		{
			...
			for(int i = 0;i<ROI_PER_SLANTBOX;i++)
			{
				...
				lIsHorz = !lIsHorz;
				Boxes[boxNumber].SFRBox[i].DesiredFreq = 67.0;
	...
	for ( int lRoi = 0; lRoi< ROI_PER_SLANTBOX; lRoi++)
	{
		for (int lBoxes = 0; lBoxes < MAX_NUMBER_BOXES; lBoxes++)
		{
			...
			{
				//Boxes[lBoxes].SFRBox[lRoi].roi.height = 36;
				cv::Point p1;
				//Boxes[lBoxes].SFRBox[lRoi].roi.x += 6;
				//if (Boxes[lBoxes].SFRBox[lRoi].roi.y > 6)
				//	Boxes[lBoxes].SFRBox[lRoi].roi.y -= 6;
	...
	wchar_t *DataVector_Hi = new wchar_t[dataArray_Hi_16BitsPerPixel->row *dataArray_Hi_16BitsPerPixel->col];
	memset(DataVector_Hi,0, dataArray_Hi_16BitsPerPixel->row *dataArray_Hi_16BitsPerPixel->col *2);
	int k_Hi = 0;
	for (int i = 0; i < dataArray_Hi_16BitsPerPixel->row; ++i)
	{
		for (int j = 0; j < dataArray_Hi_16BitsPerPixel->col; ++j)
		{
			DataVector_Hi[k_Hi] = static_cast<wchar_t>(static_cast<int>(dataArray_Hi_16BitsPerPixel->matrix[i][j])); 
			k_Hi++;
		}
	}
	...
	for (int lBoxes = 0; lBoxes < MAX_NUMBER_BOXES; lBoxes++)
	{
		for ( int lRoi = 0; lRoi< ROI_PER_SLANTBOX; lRoi++)
		{
			if ((Boxes[lBoxes].box_ctr.x != 0 ) &&  (Boxes[lBoxes].box_ctr.y != 0 ))
			{
				CalcSFR_Hi(DataVector_Hi, Boxes[lBoxes].SFRBox[lRoi], dataArray_Hi_16BitsPerPixel->col, dataArray_Hi_16BitsPerPixel->row, dataArray_Hi_16BitsPerPixel->max_gray, 0.00375, modeflags);
			}
		}
	}
...
void Score_Image(unsigned char *pBuffer, const flags_t modeflags)
{
	...
	dataArray_Hi_12BitsPerPixel->matrix = allocate_dynamic_matrix(dataArray_Hi_12BitsPerPixel->row, dataArray_Hi_12BitsPerPixel->col);
	//dataArray_Hi_12BitsPerPixel = readPGM(szChar, dataArray_Hi_12BitsPerPixel);
	// Convert to 12 bit image
	...
	for (int i = 0; i < dataArray_Hi_12BitsPerPixel->row; ++i)
	{
		for (int j = 0; j < dataArray_Hi_12BitsPerPixel->col; ++j)
		{
			...
			dataArray_Hi_12BitsPerPixel->matrix[i][j] = static_cast<wchar_t>(static_cast<int>(pixel)); 
			pBuffer_Counter += 2;			
		}
	}
	...
	dataArray_Hi_12BitsPerPixel->max_gray = 4095;
	//strcat(szChar, "_12bits.pgm");
	if (((modeflags.WriteImageLevel >=0)&& ((modeflags.WriteImageLevel < 2)&&(modeflags.WriteImageLevel > 3) )&& (lKey == NO_KEY_PRESSED)) || (((lKey == 0x73)|| (lKey == 0x53))&& ((modeflags.WriteImageLevel == 2)||(modeflags.WriteImageLevel == 3))))
	{
		writePGM_16("test_12bits.pgm", dataArray_Hi_12BitsPerPixel,0,0,dataArray_Hi_12BitsPerPixel->row,dataArray_Hi_12BitsPerPixel->col,dataArray_Hi_12BitsPerPixel->max_gray,DeAllocate12BPP_Memory);
		cout << "test_12bits.pgm" << " is Created" << endl; 
	}
	////////////////////////////////////////////////////
	// C O M M O N   S T A R T I N G   P O I N T
	////////////////////////////////////////////////////
	...
	cv::Mat lImageData_32BitsPerPixel;
	lImageData_12BitsPerPixel.convertTo(lImageData_32BitsPerPixel,CV_32F);
	cv::Mat lImageDataDisplay_12BitsPerPixel = cv::Mat(dataArray_Hi_12BitsPerPixel->row, dataArray_Hi_12BitsPerPixel->col, CV_16UC1, pBuffer);
	if (lImageDataDisplay_12BitsPerPixel.rows == 0 )
	{
		cout <<"error reading the 12 bits Per Pixel Display Image" << endl;
		cout << "Press Any Key to Exit" << endl;
...
//		cout << endl << "Detected ROIs" << endl;
	...
	//{
	//	strcat(szChar_DD, "_ObjectDetection.pgm");
	//	bool resu= ImageWrite(szChar_DD,lImageDataDisplay_12BitsPerPixel);
	//}
...
//	cout << endl << "Applying RCC Filter and Scores Slant-Edges" << endl;
	...
	for (int lBoxes = 0; lBoxes < MAX_NUMBER_BOXES; lBoxes++)
	{
		...
		{
			if ((Boxes[lBoxes].box_ctr.x != 0 ) &&  (Boxes[lBoxes].box_ctr.y != 0 ))
				CalcSFR_Hi(DataVector_Hi, Boxes[lBoxes].SFRBox[lRoi], dataArray_Hi_12BitsPerPixel->col, dataArray_Hi_12BitsPerPixel->row, dataArray_Hi_12BitsPerPixel->max_gray, 0.00375, modeflags);
		}
	}
	...
	if(Display)
	{
		...
		{
			//mTRWCalc.UpdateROIs(half_width,half_height,mTRWCalc.m_BoxesFilteredSFR[lBoxNumber]);
		...
		//cv::Mat pano = cv::imread(result_name , CV_LOAD_IMAGE_GRAYSCALE);
		//cv::convertScaleAbs(lImageDataDisplay_8BitsPerPixel,lImageDataDisplay_8BitsPerPixel);
		imshow( "DisplayWindow", lImageDataDisplay_8BitsPerPixel );

		if ((((lKey == 0x73)|| (lKey == 0x53))&& ((modeflags.WriteImageLevel == 2)||(modeflags.WriteImageLevel == 3)))|| (((modeflags.WriteImageLevel < 2)&&(modeflags.WriteImageLevel > 3))&& (lKey == NO_KEY_PRESSED)))
		{
			ostringstream convert;			// stream used for the conversion
			convert << ImageCounter;		// insert the textual representation of 'Number' in the characters in the stream
			//Result = convert.str();		// set 'Result' to the contents of the stream
			cv::string result_name = "image_" + convert.str()  + ".pgm";
			ImageWrite(result_name, lImageDataDisplay_8BitsPerPixel);
		}
	}
...
int main(int argc, char* argv[])
{
	...
	// Attempt to load the templates from the directory where the executable is
	if (!InitTemplates(szCharWithoutExePath))
	{
	...
	if ( strcmp(argv[argc-1], "live") ) {
...
#ifndef NO_CAMERA
		// Test to make sure we have at least one camera available
		if (ap_DeviceProbe(NULL)) {
			...
			if (!hCamera)
			{
				printf("Could not get handle to a camera (Err #: %d). \n",ap_GetLastError());
				ap_Finalize();
				DelayExit(1);
			}
		} // End camera initiliazation

		// Open Display once we've got a valid camera handle
		if (Display) { cv::namedWindow( "DisplayWindow", cv::WINDOW_AUTOSIZE ); }
...
//			frameLength = ap_GrabFrame(hCamera, pBuffer, (ap_u32)nBufferSize);
			...
			{
				// The following keeps writing images to output file; Should it?
