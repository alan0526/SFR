...
int findstarnumber(cv::Point center, int width, int height)
{
	int x = center.x - width/2;
	int y = center.y - height/2;
	int diagsqr = (width*width + height*height)/4; 
	int radiusqr = x*x + y*y;
	if(radiusqr < diagsqr/25)
	{
		return 0;
	}
	else if(radiusqr < (diagsqr*4)/9)
	{
		if(x>0&y<0) return 1;
...
int LocateMultipleFeaturesDetection2(cv::Mat img, cv::Mat img_display, int width, int height, const flags_t modeflags)
{
	...
	{
		cv::Scalar color = cv::Scalar( 4000, 255, 255 );
			...
			((boundRect[i].width > 100) && ( modeflags.Distance == 0)))  // 3.5 meters
		{
			...
			if(boundRect[i].y < (int)(height/3))
			{
				...
				else if (boundRect[i].x >= (int)((2-modeflags.fivemwall)*width/3))
				{
			...
			else if(boundRect[i].y >= (int)((2-modeflags.fivemwall)*height/3))
			{
				if (boundRect[i].x <= (int)(width/3))
				{
...
int LocateMultipleFeatures(cv::Mat img, cv::Mat img_display, int width, int height, const flags_t modeflags)
{
	...
	if(modeflags.typeoflens  == 2) //Blacken the two side walls of one image for wide FOV lens
	{
		cv::rectangle(img,cv::Point(0,0),cv::Point(275,DEFAULT_IMAGE_HEIGHT-1),CV_RGB(0,0,0),-1,8);
		cv::rectangle(img,cv::Point(DEFAULT_IMAGE_WIDTH-243,0),cv::Point(DEFAULT_IMAGE_WIDTH-1,DEFAULT_IMAGE_HEIGHT-1),CV_RGB(0,0,0),-1,8);
		//namedWindow( "Trim", cv::WINDOW_AUTOSIZE);
        //imshow( "Trim", img);
		//cv::waitKey(0);
	}
	...
	if(modeflags.typeoflens==2)
	{
		lTotalDetectedBoxes = 5;
	}
	...
	if(modeflags.star)  //star target, just find all of them
	{
		lTotalDetectedBoxes = 9;
		int boxnumber;
		while(lTotalDetectedBoxes>0 && lMaxIteration>0)
		{
			lMaxIteration--;
			...
			boxnumber = findstarnumber(TargetCenterDetection,width,height);

			if (!m_DetectedTargetCenterBoxes[boxnumber])
			{
			m_AllTargetCenterBoxes[boxnumber] = TargetCenterDetection;
			m_DetectedTargetCenterBoxes[boxnumber] = true;
			lTotalDetectedBoxes--;
			}
			else
			{
	...
	{while (true)
	{
		...
		if (maxVal >= threshold)
		{
			...
			if(modeflags.typeoflens!=2) //for narrow and main lens target
			{
			if(TargetCenterDetection.y < (int)(height/3))
			{
				if (TargetCenterDetection.x <= (int)(width/3))
			...
			else  //modeflags.typeoflens = 2, wide FOV lens
			{
				//center quad
               if(TargetCenterDetection.x >= (int)(width*0.4) && TargetCenterDetection.x <= (int)(width*0.6) && TargetCenterDetection.y >= (int)(height*0.4) && TargetCenterDetection.y <= (int)(height*0.6))
			   {
				   if (!m_DetectedTargetCenterBoxes[0])
					{
						m_AllTargetCenterBoxes[0] = TargetCenterDetection;
			   ...
			   //top-right quad
			   if(TargetCenterDetection.x > (int)(width*0.6) && TargetCenterDetection.y < (int)(height*0.4))
			   {
				   if (!m_DetectedTargetCenterBoxes[1])
					{
						m_AllTargetCenterBoxes[1] = TargetCenterDetection;
			   ...
			   //bottom-right quad
			   if(TargetCenterDetection.x > (int)(width*0.6) && TargetCenterDetection.y > (int)(height*0.6))
			   {
				   if (!m_DetectedTargetCenterBoxes[2])
					{
						m_AllTargetCenterBoxes[2] = TargetCenterDetection;
			   ...
			   //bottom-left quad
			   if(TargetCenterDetection.x < (int)(width*0.4) && TargetCenterDetection.y > (int)(height*0.6))
			   {
				   if (!m_DetectedTargetCenterBoxes[3])
					{
						m_AllTargetCenterBoxes[3] = TargetCenterDetection;
			   ...
			   //top-left quad
			   if(TargetCenterDetection.x < (int)(width*0.4) && TargetCenterDetection.y < (int)(height*0.4))
			   {
				   if (!m_DetectedTargetCenterBoxes[4])
					{
						m_AllTargetCenterBoxes[4] = TargetCenterDetection;
	...
	while (true)
	{
		...
		if (maxVal >= threshold)
		{
			...
			else   //modeflags.typeoflens = 2, wide FOV lens target
			{
				if(TargetCenterDetection.x >= (int)(width*0.4) && TargetCenterDetection.x <= (int)(width*0.6) && TargetCenterDetection.y >= (int)(height*0.4) && TargetCenterDetection.y <= (int)(height*0.6))
			   {
				   if (!m_DetectedTargetCenterBoxes[0])
...
bool InitializeROIs(string filePath, const flags_t modeflags)
{
	...
	if(modeflags.OPTO)
		ConfigurationXML.open(filePath + "\\xmlfiles\\Quadopto_main.xml", cv::FileStorage::READ);
	else if(!modeflags.typeoflens)
	ConfigurationXML.open(filePath + "\\xmlfiles\\Quadmain.xml", cv::FileStorage::READ);
	else if(modeflags.typeoflens == 1)
	ConfigurationXML.open(filePath + "\\xmlfiles\\Quadnarrow.xml", cv::FileStorage::READ);
	else if(modeflags.typeoflens == 2)
	ConfigurationXML.open(filePath + "\\xmlfiles\\Quadwide.xml", cv::FileStorage::READ);
...
double CalcSFR_Hi(wchar_t* ptr_img, Roi_for_SFR & SFRBox, int width, int height, int grey_Level, double MM_PER_PIX, const flags_t modeflags, const int redchan)
{
  ...
  // Check for this here and set value to 0.0 which will fail tests.
	...
	else
	{
		...
		SFRBox.SFRValue =SFRBox.Edgeangle = err; 
		return (double)err;
	}
} // End for CalcSFR_Hi()
...
void OverlayROIs(UINT8* DisplayImg, int width, int height, SFRPlus & SB)
{
	...
	for(i=0;i<ROI_PER_SLANTBOX;i++)
	{
		if(SB.SFRBox[i].inRange)
		{
			...
			sprintf(ScoreSTR,"%4.2f\nEA:%3.2f",ceilf(SB.SFRBox[i].SFRValue * 100) / 100, SB.SFRBox[i].Edgeangle);
...
void OverlayROIs_16BPP(UINT8* DisplayImg, int width, int height, SFRPlus & SB)
{
	...
	for(i=0;i<ROI_PER_SLANTBOX;i++)
	{
		if(SB.SFRBox[i].inRange)
		{
			...
			rectangle( img_display, s , e, (128,128,128), 1, 8, 0 ); 
			sprintf(ScoreSTR,"%4.2f\nEA:%3.2",ceilf(SB.SFRBox[i].SFRValue * 100) / 100,SB.SFRBox[i].Edgeangle);
...
void Score_Image(int argc, char* argv[], flags_t &modeflags)
{
	...
	if (dataArray_Hi_16BitsPerPixel->max_gray == 65535&&!modeflags.Mobileye) { bits2shift = 4; } // bits2shift should = 4 if max_gray = 65535
	...
	*/
	...
	if (modeflags.WriteImageLevel >= 0 && !modeflags.Mobileye)
	{
	...
	cv::Mat lImageData_12BitsPerPixel;
	if(modeflags.Mobileye)
	{
		lImageData_12BitsPerPixel = cv::imread(szChar, CV_LOAD_IMAGE_ANYDEPTH);
	}
	else
	{
		lImageData_12BitsPerPixel = cv::imread(szChar_DD, CV_LOAD_IMAGE_ANYDEPTH);
	}
...
cv::minMaxLoc(lImageData_12BitsPerPixel, &minVal, &maxVal, &minLoc, &maxLoc);

	//check if the 12 bits images are actually 8 bits
	if(maxVal<256) //8bit image
	{
		lImageData_12BitsPerPixel = lImageData_12BitsPerPixel*16;
		//cv::minMaxLoc(lImageData_12BitsPerPixel, &minVal, &maxVal, &minLoc, &maxLoc);
	}
	...
	if (modeflags.Demosaic == 1) {
		...
		for (int row=0; row<lImageData_32BitsPerPixel.rows; row+=2) 
		{
			for (int col=0; col<lImageData_32BitsPerPixel.cols; col+=2)
			{
	...
	case 1:
		{
			...
			for (int i = 0; i < lImageDataDisplay_12BitsPerPixel.rows; ++i)
			{
				...
				if (lSaturated)
					...
					//doing nothing when it is saturated
					//lSaturated = false;

					break;					
			}
			*/
			if(!lSaturated || !modeflags.saturationswitch)
				lNumberOfDetectedBoxes = LocateMultipleFeatures(lImageData_32BitsPerPixel, lImageDataDisplay_32BitsPerPixel, lImageDataDisplay_32BitsPerPixel.cols, lImageDataDisplay_32BitsPerPixel.rows, modeflags);
		}
		break;
	case 2:
		{
	...
	if (lSaturated && modeflags.lRunningMode==2 &&modeflags.saturationswitch)
	{
	...
	if (modeflags.lTargetDetection != 3)
	{
		...
		InitializeROIs(szCharWithoutExePath,modeflags);
	}
	else
	{
		...
		for (int boxNumber = 0; boxNumber < MAX_NUMBER_BOXES; boxNumber++)
		{
			...
			for(int i = 0;i<ROI_PER_SLANTBOX;i++)
			{
				...
				Boxes[boxNumber].SFRBox[i].x_Delta = 0;
				Boxes[boxNumber].SFRBox[i].y_Delta = 0;
...
//#pragma omp parallel for //collapse(2)
	for (lBoxes = 0; lBoxes < MAX_NUMBER_BOXES; lBoxes++)
	{
//		noft = omp_get_num_threads(); 
		for (lRoi = 0; lRoi< ROI_PER_SLANTBOX; lRoi++)
		{
			if ((Boxes[lBoxes].box_ctr.x != 0 ) &&  (Boxes[lBoxes].box_ctr.y != 0 ))
	...
	//MyFile <<endl;
		...
		//create a new row of edge angles
	if(modeflags.showedgeangle)
	{

	MyFile << "Edgeangle," ;

		//Box locations and edge angles
		    for (int lBoxes = 0; lBoxes < MAX_NUMBER_BOXES; lBoxes++)
		{
			MyFile << Boxes[lBoxes].box_ctr.x << "," << Boxes[lBoxes].box_ctr.y << ",";
			for ( int lRoi = 0; lRoi< ROI_PER_SLANTBOX; lRoi++)
			{
					MyFile <<Boxes[lBoxes].SFRBox[lRoi].Edgeangle << ",";
			}
		}
	}
	MyFile<<endl;
...
void Score_Image(unsigned char *pBuffer, flags_t &modeflags)
{
	bool lSaturated = false;
	PGMData* dataArray_Hi_12BitsPerPixel = new PGMData();
	InitializeBoxes();
	...
	char savefn2[100] = "C:\\imagebuffer\\liveimage_";
	char s[100];
	time_t now;
	struct tm * p;
	//#pragma omp parallel for
	...
	if(modeflags.savelive)
	{
		now = time(NULL);
		p = localtime(&now);
		sprintf(s,"%d",modeflags.increment);
		strcat(savefn2,s);

		strcat(savefn2,".pgm");
		writePGM(savefn2, dataArray_Hi_12BitsPerPixel);
		modeflags.increment++;
...
//		if (m_DetectedTargetCenterBoxes[lBoxes]) {
	...
	if(Display)
	{
		...
		//write the image
		//ImageWrite("8bittoshow.pgm",lImageDataDisplay_8BitsPerPixel);
		
		imshow( "Live SFRscoring", lImageDataDisplay_8BitsPerPixel );
		...
		( ((modeflags.WriteImageLevel < 2) && (modeflags.WriteImageLevel > 3)) && (lKey == NO_KEY_PRESSED)) )
	{
		...
		if(modeflags.showedgeangle)
		{
		//create another row to insert the row of edge angles
		MyFile <<"EdgeAngle,";
...
//		pBuffer  = (unsigned char *)malloc(nBufferSize);
		...
		if (Display) { cv::namedWindow( "Live SFRscoring",cv::WINDOW_AUTOSIZE); }
		break;
		case OMNI:
			nWidth = 1280;
			nHeight = 960;
		...
		while (true)
		{
			...
			switch (lKey)
			{
				...
				case 'V':
					modeflags.savelive = true;
					modeflags.num_frames++;
					printf("Adding 1 frame to queue, saving 1 frame to disc\n");
					break;
				// Quit
				case 'q':
				case 'Q':
